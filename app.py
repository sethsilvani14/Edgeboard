...
MODEL_VERSION = "CFB baseline 0.2"
HOME_FIELD = 2.5
MARGIN_SD = 14.0
TOTAL_SD = 17.0
...
with left:
    st.title("📊 EdgeBoard")
    st.caption(f"V3 • {MODEL_VERSION} • consensus-market architecture")
with right:
    st.write("")
    st.write("")
...
ready = bool(odds_key and cfbd_key)
year = season_year()
...
if not ready:
    st.warning("V3 is installed, but live mode needs two API keys in Streamlit Secrets: ODDS_API_KEY and CFBD_API_KEY.")
...
if ready:
    try:
        with st.spinner("Loading CFB market + model data…"):
            events, quota = fetch_odds(odds_key)
            ...
            board, unmatched = build_board(events, stats, srs, nat_avg)
...
with best_tab:
    ...
    if not ready:
        ...
        if quota.get("remaining"):
            st.caption(f"Odds API quota remaining: {quota['remaining']} • refresh cost: {quota.get('last', '—')} credits")
        ...
        for idx, row in view.iterrows():
            ...
            with st.expander("Market + best available price"):
                ...
                st.caption("Consensus is built from all returned US books. The book is a price source, not the model's identity.")
...
1. Pulls current **moneyline, spread and total** markets from multiple available US books.
...
with setup_tab:
    ...
    if ready:
        ...
        st.markdown("""
**1. The Odds API** → live NCAAF prices from the US books available in the feed.  
...
Save the secrets and reboot/refresh the app.
    ...
    st.info("EdgeBoard does not depend on FanDuel, bet365, or any single sportsbook. It uses the books returned by the odds feed to construct consensus, then identifies the best available offer as a separate execution detail.")
    st.markdown("#### V3 → V4")
...
st.caption("EdgeBoard V3 • CFB baseline • consensus first • best price second • never fabricates missing live odds")
