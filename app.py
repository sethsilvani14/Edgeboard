...
NCAA_LEAGUE_ID = 2
MODEL_VERSION = "CFB baseline 0.3 • API-Sports free stack"
HOME_FIELD = 2.5
MARGIN_SD = 14.0
TOTAL_SD = 17.0
...
def fetch_api_sports_schedule(api_key: str, year: int):
    """One NCAA schedule request; odds are fetched only for upcoming FBS-matchable games."""
    r = requests.get(
        f"{API_SPORTS_BASE}/games",
        params={"league": NCAA_LEAGUE_ID, "season": year},
    ...
    if payload.get("errors"):
        raise RuntimeError(f"API-Sports schedule error: {payload['errors']}")
    return payload.get("response", []), quota_from_headers(r.headers)


def quota_from_headers(headers):
    lower = {str(k).lower(): v for k, v in headers.items()}
    return {
        "remaining": lower.get("x-ratelimit-requests-remaining") or lower.get("x-ratelimit-remaining"),
        "limit": lower.get("x-ratelimit-requests-limit") or lower.get("x-ratelimit-limit"),
    }


def api_game_to_event(g):
    game = g.get("game", {})
    teams = g.get("teams", {})
    ...
    if ts:
        commence = datetime.fromtimestamp(int(ts), tz=timezone.utc).isoformat()
    else:
        d, t = date.get("date"), date.get("time", "00:00")
        commence = f"{d}T{t}:00Z" if d else None
    return {"id": str(gid), "api_game_id": gid, "home_team": home, "away_team": away, "commence_time": commence, "bookmakers": []}


def upcoming_fbs_events(schedule, stats, srs, days=7):
    now = datetime.now(timezone.utc)
    candidates = set(stats) | set(srs)
    out = []
    for g in schedule:
        event = api_game_to_event(g)
        if not event["home_team"] or not event["away_team"] or not event["api_game_id"]:
            continue
        dt = pd.to_datetime(event.get("commence_time"), utc=True, errors="coerce")
        if pd.isna(dt):
            continue
        hours = (dt.to_pydatetime() - now).total_seconds() / 3600
        if hours < -6 or hours > days * 24:
            continue
        # This protects the 100/day free quota by skipping FCS-only games before /odds calls.
        if not match_team(event["home_team"], candidates) or not match_team(event["away_team"], candidates):
            continue
        out.append(event)
    return out
...
def _side_from_value(value_text, home, away):
    raw = str(value_text or "").strip()
    low = raw.lower()
    if low in {"home", "1", "team 1"} or home.lower() in low:
        return home
    if low in {"away", "2", "team 2"} or away.lower() in low:
        return away
    return None


def normalize_api_sports_odds(event, odds_payload):
    """Convert API-Sports' bookmaker/bet/value structure into EdgeBoard's internal format."""
    home, away = event["home_team"], event["away_team"]
    normalized_books = []
    responses = odds_payload.get("response", []) if isinstance(odds_payload, dict) else []
    for snap in responses:
        for book in snap.get("bookmakers", []) or []:
            markets = {"h2h": [], "spreads": [], "totals": []}
            for bet in book.get("bets", []) or []:
                bet_name = str(bet.get("name", ""))
                bn = bet_name.lower()
                # Keep full-game core markets only; halves/quarters/teams come later.
                secondary = any(x in bn for x in ("quarter", "half", "1st", "2nd", "team total", "home total", "away total"))
                for val in bet.get("values", []) or []:
                    label = str(val.get("value", ""))
                    odd_raw = val.get("odd") or val.get("odds")
                    if odd_raw in (None, ""):
                        continue
                    try:
                        price = decimal_to_american(odd_raw)
                    except Exception:
                        continue

                    if not secondary and ("moneyline" in bn or bn in {"home/away", "winner", "match winner"}):
                        side = _side_from_value(label, home, away)
                        if side:
                            markets["h2h"].append({"name": side, "price": price})
                    elif not secondary and ("handicap" in bn or "spread" in bn):
                        side = _side_from_value(label, home, away)
                        point = val.get("handicap") or val.get("point") or _first_number(label)
                        if side and point is not None:
                            markets["spreads"].append({"name": side, "price": price, "point": float(point)})
                    elif not secondary and ("over/under" in bn or "total" in bn):
                        ll = label.lower()
                        side = "Over" if "over" in ll else "Under" if "under" in ll else None
                        point = val.get("handicap") or val.get("point") or _first_number(label)
                        if side and point is not None:
                            markets["totals"].append({"name": side, "price": price, "point": abs(float(point))})

            packed = [{"key": k, "outcomes": v} for k, v in markets.items() if v]
            if packed:
                normalized_books.append({
                    "key": str(book.get("id", book.get("name", "book"))),
                    "title": book.get("name", "Book"),
                    "markets": packed,
                })
    event = dict(event)
    event["bookmakers"] = normalized_books
    return event


@st.cache_data(ttl=86400, show_spinner=False)
def fetch_api_sports_odds_batch(api_key: str, events_tuple):
    """Free-tier safe snapshot: at most one /odds request per FBS game, cached for 24h."""
    events = [
        {"id": x[0], "api_game_id": x[1], "home_team": x[2], "away_team": x[3], "commence_time": x[4], "bookmakers": []}
        for x in events_tuple
    ]
    out, remaining = [], None
    for event in events:
        r = requests.get(
            f"{API_SPORTS_BASE}/odds",
            params={"game": event["api_game_id"]},
            headers={"x-apisports-key": api_key},
            timeout=20,
        )
        ...
        if payload.get("errors"):
            continue
        q = quota_from_headers(r.headers)
        remaining = q.get("remaining") or remaining
        out.append(normalize_api_sports_odds(event, payload))
    return out, {"remaining": remaining, "calls": len(out) + 1}
...
with left:
    st.title("📊 EdgeBoard")
    st.caption(f"V3.1 • {MODEL_VERSION} • consensus-market architecture")
with right:
    st.write("")
    st.write("")
...
api_sports_key = secret("API_SPORTS_KEY")
cfbd_key = secret("CFBD_API_KEY")
ready = bool(api_sports_key and cfbd_key)
year = season_year()
...
if not ready:
    st.warning("V3.1 is installed, but live mode needs two API keys in Streamlit Secrets: API_SPORTS_KEY and CFBD_API_KEY.")
...
if ready:
    try:
        with st.spinner("Loading CFB market + model data…"):
            ...
            schedule, schedule_quota = fetch_api_sports_schedule(api_sports_key, year)
            eligible = upcoming_fbs_events(schedule, stats, srs, days=7)
            # Tuples make the cached batch function hashable and stable in Streamlit.
            event_tuples = tuple(
                (e["id"], e["api_game_id"], e["home_team"], e["away_team"], e["commence_time"])
                for e in eligible
            )
            normalized_events, quota = fetch_api_sports_odds_batch(api_sports_key, event_tuples)
...
with best_tab:
    ...
    if not ready:
        ...
        if quota.get("remaining"):
            st.caption(f"API-Sports requests remaining today: {quota['remaining']} • snapshot used about {quota.get('calls', '—')} calls")
        ...
        for idx, row in view.iterrows():
            ...
            with st.expander("Market + best available price"):
                ...
                st.caption("Consensus is built from the bookmakers API-Sports returns for each game. The book is a price source, not the model's identity.")
...
1. Pulls available **moneyline, spread and total** markets from API-Sports bookmakers for upcoming NCAA games.
...
with setup_tab:
    ...
    if ready:
        ...
        st.markdown("""
**1. API-Sports** → NCAA schedule + pregame bookmaker odds.  
...
Save the secrets and reboot/refresh the app.
    ...
    st.info("API-Sports allows 100 requests/day on the free American-football API. EdgeBoard first filters the NCAA schedule to FBS-matchable games, then makes one odds call per game and caches that full snapshot for 24 hours. The Refresh view button does not spend more API calls.")
    st.markdown("#### Architecture")
    st.info("EdgeBoard does not depend on FanDuel, bet365, or any single sportsbook. It uses the bookmakers returned by API-Sports to construct consensus, then identifies the best available offer as a separate execution detail. The free plan is protected with one cached full-board snapshot per day.")
    st.markdown("#### V3 → V4")
...
st.caption("EdgeBoard V3.1 • CFB baseline • API-Sports free stack • consensus first • never fabricates missing odds")
